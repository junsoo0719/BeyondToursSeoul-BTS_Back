...
public class MeSavedController {
    ...
    @GetMapping("/events")
    public ResponseEntity<List<SavedEventResponse>> savedEvents(
            @AuthenticationPrincipal Jwt jwt,
            @Parameter(description = "언어 코드. ko(기본)/en/zh/ja — 행사명·주소 번역에 사용")
            @RequestHeader(value = "Accept-Language", required = false, defaultValue = "ko") String acceptLanguage) {
        if (jwt == null) {
            return ResponseEntity.status(401).build();
        }
        UUID userId = UUID.fromString(jwt.getSubject());
        return ResponseEntity.ok(userSavedEventService.listSaved(userId, acceptLanguage));
    }
